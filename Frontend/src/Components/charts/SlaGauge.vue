...
const chartOptions = computed(() => ({
  ...
  colors: ['#6A9A2A'],
  plotOptions: { radialBar: { startAngle: -135, endAngle: 135, hollow: { size: '66%' }, track: { background: '#EEF1E9', strokeWidth: '100%', margin: 5 }, dataLabels: { name: { show: true, offsetY: 22, color: '#64748B', fontSize: '12px', fontWeight: 600 }, value: { show: true, offsetY: -18, color: '#111827', fontSize: '32px', fontWeight: 800, formatter: (value) => `${Math.round(value)}%` } } } },
  labels: [content.value.objective],
  stroke: { lineCap: 'round' },
  fill: { type: 'gradient', gradient: { shade: 'light', type: 'horizontal', shadeIntensity: 0.3, gradientToColors: ['#B6C65B'], inverseColors: false, opacityFrom: 1, opacityTo: 1, stops: [0, 100] } },
...
.sla-item { display: flex; align-items: center; gap: 10px; padding: 11px; background: #fafbf7; border: 1px solid #edf0e8; border-radius: 12px; }
.sla-item div { display: flex; flex-direction: column; }
.sla-item strong { color: #111827; font-size: 15px; }
.sla-item small { margin-top: 2px; color: #64748b; font-size: 10px; }
.status-dot { width: 10px; height: 10px; flex-shrink: 0; border-radius: 50%; }
.status-dot.success { background: #6a9a2a; }
.status-dot.danger { background: #e31e24; }
